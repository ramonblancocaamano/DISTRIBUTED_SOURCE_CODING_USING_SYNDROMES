...
% 
% @about: TRELLIS CODE QUANTIZER(2bit).
%%
function [output, w_tcq] = tcq_encoder2(coset,input)

    % DECLARATION OF VARIABLES.
    frame_length = length(input);
    
    % TRANSITIONS: CENTROID & CODE & METRIC.
    centroid = zeros(4,frame_length); % STATE/CENTROID.
    code = zeros(4,2*frame_length); % STATE/BIT.
    cost = [0 0 0 0]; % STATE.
    ...
    for index = 1 : frame_length
        ...
        if index == 1             
            ...
            transition(2).code(1,2) = position_S0 - 1;  
            %% STATE 1.
            ...
            transition(2).code(2,2) =  position_S2 - 1;
            continue;
        elseif index == 2
            %% STATE 0.
            % TRANSITION 1: STATE 0 - S0, CODE &0.
            ...
            transition(2).code(1,3) = 0;
            transition(2).code(1,4) = position_S0 - 1;
            %% STATE 1.
            % TRANSITION 1: STATE 0 - S2, CODE &1.
            ...
            transition(2).code(2,3) = 1;
            transition(2).code(2,4) = position_S2 - 1; 
            %% STATE 2.
            ...
            transition(2).code(3,3) = 0;
            transition(2).code(3,4) = position_S1 - 1;
            %% STATE 3.
            ...
            transition(2).code(4,3) = 1;
            transition(2).code(4,4) = position_S3 - 1;
            continue;
        end
        %%
        % STATE 0.
        ...
        transition(2).code(1,index*2 - 1) = 0;
        switch state
            case 1
                transition(2).centroid(1,index) = coset.S0(position_S0);
                transition(2).code(1,index*2) = position_S0 - 1;
            case 3
                transition(2).centroid(1,index) = coset.S2(position_S2);
                transition(2).code(1,index*2) = position_S2 - 1;
        end
        %%
        % STATE 1.
        % TRANSITION 1: STATE 0 - S2, CODE &1. 
	...
	transition(2).code(2,:) = transition(1).code(state,:);
        transition(2).code(2,index*2 - 1) = 1;
        switch state
            case 1
                transition(2).centroid(2,index) = coset.S2(position_S2);
		transition(2).code(2,index*2) = position_S2 - 1;
            case 3
                transition(2).centroid(2,index) = coset.S0(position_S0);
		transition(2).code(2,index*2) = position_S0 - 1;
        end      
	%%
        % STATE 2.
        % TRANSITION 2: STATE 1 - S1, CODE &0.
	...
	transition(2).code(3,:) = transition(1).code(state,:);
        transition(2).code(3,index*2 - 1) = 0;
        switch state
            case 2
                transition(2).centroid(3,index) = coset.S1(position_S1);
		transition(2).code(3,index*2) = position_S1 - 1;
            case 4
                transition(2).centroid(3,index) = coset.S3(position_S3);
		transition(2).code(3,index*2) = position_S3 - 1;
        end
        %%
        % STATE 3.
        % TRANSITION 2: STATE 1 - S3, CODE &1.
	...
	transition(2).code(4,:) = transition(1).code(state,:);
        transition(2).code(4,index*2 - 1) = 1;
        switch state
            case 2
                transition(2).centroid(4,index) = coset.S3(position_S3);
		transition(2).code(4,index*2) = position_S3 - 1;
            case 4
                transition(2).centroid(4,index) = coset.S1(position_S1);
		transition(2).code(4,index*2) = position_S1 - 1;
        end
    end
